...
//     ctrl->sockfd = accept(ctrl->listen_fd, (struct sockaddr*)&client_addr, &len);
//     if (ctrl->sockfd < 0) {
//         perror("accept");
//         return -1;
//     }
...
int wait_flag(struct control_channel *ctrl, ctrl_flag_t expected_flag) {
    ...
    while (1) {
        ...
        int client_fd = event.data.fd;  // 这是实际有数据的fd

        // 一对一场景：event.data.fd必然是ctrl->sockfd
        while (1) {
            int ret = recv_flag(client_fd, &flag);
            if (ret < 0) {
                if (errno == EAGAIN || errno == EWOULDBLOCK) {
                    break; // 没有更多数据
                }

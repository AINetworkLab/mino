...
static doca_error_t send_string_callback(void *param, void *config) {
  ...
  if (len == MAX_ARG_SIZE) {
    DOCA_LOG_ERR("Entered send string exceeded buffer size: %d",
                 MAX_USER_ARG_SIZE);
    return DOCA_ERROR_INVALID_VALUE;
  }
...
static doca_error_t read_string_callback(void *param, void *config) {
  ...
  if (len == MAX_ARG_SIZE) {
    DOCA_LOG_ERR("Entered read string exceeded buffer size: %d",
                 MAX_USER_ARG_SIZE);
    return DOCA_ERROR_INVALID_VALUE;
  }
  /* The string will be '\0' terminated due to the strnlen check above */
...
static doca_error_t num_connections_param_callback(void *param, void *config) {
  ...
  if (num_connections > MAX_NUM_CONNECTIONS) {
    DOCA_LOG_ERR("Max number of connections must be <= [%d]",
                 MAX_NUM_CONNECTIONS);
    return DOCA_ERROR_INVALID_VALUE;
  }
...
doca_error_t register_rdma_num_connections_param(void) {
  ...
  snprintf(param_desc, MAX_ARG_SIZE, "%s%d%s",
           "num_connections for DOCA RDMA (optional), max connections number "
           "must be <= ",
           MAX_NUM_CONNECTIONS, " in this sample");
  doca_argp_param_set_description(num_connections_param, param_desc);
  doca_argp_param_set_callback(num_connections_param,
...
static doca_error_t transport_type_param_callback(void *param, void *config) {
  ...
  if (strcasecmp(type, "RC") == 0)
    ...
    DOCA_LOG_ERR(
        "Entered wrong RDMA transport_type, the accepted RDMA transport_type "
        "are: "
        "RC, rc, "
        "DC, dc");
    return DOCA_ERROR_INVALID_VALUE;
...
static doca_error_t cm_addr_param_callback(void *param, void *config) {
  ...
  if (addr_len == SERVER_ADDR_LEN) {
    DOCA_LOG_ERR("Entered server address exceeded buffer size: %d",
                 SERVER_ADDR_LEN);
    return DOCA_ERROR_INVALID_VALUE;
  }
...
static doca_error_t cm_addr_type_param_callback(void *param, void *config) {
  ...
  if (type_len == SERVER_ADDR_TYPE_LEN) {
    DOCA_LOG_ERR("Entered server address type exceeded buffer size: %d",
  ...
  if (strcasecmp(type, "ip4") == 0 || strcasecmp(type, "ipv4") == 0)
    ...
    DOCA_LOG_ERR(
        "Entered wrong server address type, the accepted server address type "
        "are: "
...
static doca_error_t register_rdma_cm_params(void) {
  ...
  doca_argp_param_set_description(
      use_rdma_cm_param, "Whether to use rdma-cm or oob to setup connection");
  ...
  doca_argp_param_set_description(
      cm_addr_type_param,
      "Rdma cm server device address type: IPv4, IPv6 or GID");
...
doca_error_t register_rdma_common_params(void) {
  ...
  doca_argp_param_set_description(
      ...
      "Local descriptor file path that includes the local connection "
      "descriptor, to be copied to the remote program, "
      "used only when not using the use-rdma-cm flag");
  doca_argp_param_set_callback(local_desc_path_param,
  ...
  doca_argp_param_set_long_name(remote_desc_path_param,
                                "remote-descriptor-path");
  doca_argp_param_set_description(
      remote_desc_path_param,
      "Remote descriptor file path that includes the remote connection "
      "descriptor, to be copied from the remote program, "
      "used only when not using the use-rdma-cm flag");
  doca_argp_param_set_callback(remote_desc_path_param,
  ...
  doca_argp_param_set_long_name(remote_resource_desc_path,
                                "remote-resource-descriptor-path");
  doca_argp_param_set_description(
      remote_resource_desc_path,
      "Remote descriptor file path that includes the remote mmap connection "
      "descriptor, to be copied from the remote program, "
      "used only when not using the use-rdma-cm flag");
  doca_argp_param_set_callback(remote_resource_desc_path,
  ...
  doca_argp_param_set_description(
      ...
      "transport_type for DOCA RDMA (RC or DC, optional), only useful for "
      "single connection out-of-band RDMA for now");
  doca_argp_param_set_callback(transport_type_param,
 ...
 * @func [in]: Function to check if a given device is capable of executing some
 * task
 * @doca_device [out]: An allocated DOCA device on success and NULL otherwise
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t open_doca_device(const char *device_name, task_check func,
  ...
  for (i = 0; i < nb_devs; i++) {
    result = doca_devinfo_get_ibdev_name(dev_list[i], ibdev_name,
                                         sizeof(ibdev_name));
    if (result != DOCA_SUCCESS ||
        (strlen(device_name) != 0 &&
         strncmp(device_name, ibdev_name, DOCA_DEVINFO_IBDEV_NAME_SIZE) != 0))
      continue;
...
doca_error_t allocate_rdma_resources(struct rdma_config *cfg,
  ...
  /* Check configuration correctness, for now, DC is only supported for
   * out-of-band single connection sample */
  if (((cfg->num_connections > 1) || (cfg->use_rdma_cm == true)) &&
      (cfg->transport_type == DOCA_RDMA_TRANSPORT_TYPE_DC)) {
    DOCA_LOG_ERR(
        "Failed to allocate RDMA resources: due to DOCA_RDMA_TRANSPORT_TYPE_DC "
        "is only supported for out-of-band single connection case for now");
    return DOCA_ERROR_INVALID_VALUE;
  }
  ...
  DOCA_LOG_INFO("DOCA device resources->doca_device: %p",
                resources->doca_device);

  TIMER_START(mmap_inv);
  // // 创建/复用共享段并注册 DOCA mmap
  // struct my_shm_region *reg = malloc(sizeof(*reg));
  // // 根据线程ID指定块号
  // // 线程0和2使用块0，线程1和3使用块1
  // int block_id = (cfg->thread_id % 2 == 0) ? 0 : 1;
  // DOCA_LOG_INFO("Thread %d using block %d", cfg->thread_id, block_id);
  // // 使用指定块ID创建或复用共享内存
  // result = my_doca_open_or_create_shm_with_block_id(
  // 	"/dev/hugepages/mino_doca_rdv_buf",
  // 	cfg->mem_len,
  // 	resources->doca_device,
  // 	reg,
  // 	block_id
  // );
  // if (result != 0) {
  // 	DOCA_LOG_ERR("Failed to create/reuse shared memory: %d", result);
  // 	free(reg);
  // 	return 1;
  // }
  // resources->reg = reg;
  // resources->mmap = reg->mmap;
  // resources->mmap_memrange = reg->addr;

  // 创建/复用共享段并注册 DOCA mmap
  struct my_shm_region *reg = malloc(sizeof(*reg));
  if (!reg) {
    DOCA_LOG_ERR("Failed to allocate memory for shm region");
    return DOCA_ERROR_NO_MEMORY;
  }

  // 使用智能分配函数
  DOCA_LOG_INFO("Thread %d requesting memory block of size %zu", cfg->thread_id,
                cfg->mem_len);
  result = my_doca_open_or_create_shm_smart("/dev/hugepages/mino_doca_rdv_buf",
                                            cfg->mem_len,
                                            resources->doca_device, reg);
  if (result != 0) {
    DOCA_LOG_ERR("Failed to create/reuse shared memory: %d", result);
    free(reg);
  ...
  DOCA_LOG_INFO("Thread %d assigned block %d of size %zu", cfg->thread_id,
                reg->block_id, reg->length);

  resources->reg = reg;
  resources->mmap = reg->mmap;
  resources->mmap_memrange = reg->addr;

  TIMER_END(mmap_inv, "mmap 映射耗时:");

  // ws doca_pe_create应该是一个 doca 的底层的代码
  result = doca_pe_create(&(resources->pe));
  if (result != DOCA_SUCCESS) {
    DOCA_LOG_ERR("Failed to set permissions to DOCA RDMA: %s",
                 doca_error_get_descr(result));
  ...
  if (result != DOCA_SUCCESS) {
    DOCA_LOG_ERR("Failed to create DOCA RDMA: %s",
                 doca_error_get_descr(result));
    goto destroy_pe;
  }
  ...
  if (resources->rdma_ctx == NULL) {
    ...
    DOCA_LOG_ERR("Failed to convert DOCA RDMA to DOCA context: %s",
                 doca_error_get_descr(result));
  ...
  if (cfg->is_gid_index_set) {
    ...
    if (result != DOCA_SUCCESS) {
      DOCA_LOG_ERR("Failed to set gid_index to DOCA RDMA: %s",
                   doca_error_get_descr(result));
      goto destroy_doca_rdma;
    }
  }
  ...
  result =
      doca_rdma_set_max_num_connections(resources->rdma, cfg->num_connections);
  if (result != DOCA_SUCCESS) {
    DOCA_LOG_ERR("Failed to set max_num_connections to DOCA RDMA: %s",
                 doca_error_get_descr(result));
    goto destroy_doca_rdma;
  }
  ...
  if (result != DOCA_SUCCESS) {
    DOCA_LOG_ERR("Failed to set RDMA transport type: %s",
                 doca_error_get_descr(result));
    goto destroy_doca_rdma;
  }
  result = doca_pe_connect_ctx(resources->pe, resources->rdma_ctx);
  if (result != DOCA_SUCCESS) {
    DOCA_LOG_ERR("Unable to set progress engine for RDMA: %s",
                 doca_error_get_descr(result));
    goto destroy_doca_rdma;
  }

  DOCA_LOG_INFO(
      "ctx=%p thread=%lu mmap=%p len=%u dev=%p rdma=%p mmap_memrange=%p",
      resources->rdma_ctx, (unsigned long)pthread_self(), resources->mmap,
      MEM_RANGE_LEN, (void *)resources->doca_device, resources->rdma,
...
destroy_doca_rdma:
  ...
  if (tmp_result != DOCA_SUCCESS) {
    DOCA_LOG_ERR("Failed to destroy DOCA RDMA: %s",
...
doca_error_t delete_file(const char *file_path) {
  ...
  if (fp) {
    ...
    if (res != 0) {
      ...
      DOCA_LOG_ERR("Failed to delete file %s: %s", file_path,
                   doca_error_get_descr(result));
    }
    fclose(fp);
  }
 ...
 * @return: DOCA_SUCCESS on success and DOCA_ERROR otherwise
 */
static doca_error_t destroy_rdma_cm_resources(
    struct rdma_resources *resources) {
  ...
  if (resources->cm_addr != NULL) {
    ...
    if (result != DOCA_SUCCESS) {
      DOCA_LOG_ERR("Failed to destroy DOCA rdma cm address: %s",
                   doca_error_get_descr(result));
      return result;
    }
  }
...
doca_error_t destroy_rdma_resources(struct rdma_resources *resources,
  ...
  DOCA_LOG_INFO("destroy_rdma_resources 函数被调用");
  doca_error_t result = DOCA_SUCCESS, tmp_result;
  ...
  if (resources->remote_mmap != NULL) {
    ...
    if (result != DOCA_SUCCESS)
      DOCA_LOG_ERR("Failed to stop DOCA remote mmap: %s",
                   doca_error_get_descr(result));

    tmp_result = doca_mmap_destroy(resources->remote_mmap);
    if (tmp_result != DOCA_SUCCESS) {
      DOCA_LOG_ERR("Failed to destroy DOCA remote mmap: %s",
                   doca_error_get_descr(tmp_result));
      DOCA_ERROR_PROPAGATE(result, tmp_result);
    }
  }
  ...
  if (resources->cfg->use_rdma_cm == true) {
    ...
    if (tmp_result != DOCA_SUCCESS) {
      DOCA_LOG_ERR("Failed to destroy rdma cm resources: %s",
                   doca_error_get_descr(tmp_result));
      DOCA_ERROR_PROPAGATE(result, tmp_result);
    }
  }
  ...
  //  if (tmp_result != DOCA_SUCCESS) {
  // 	 DOCA_LOG_ERR("Failed to stop DOCA mmap: %s",
  ...
  /* Free remote mmap descriptor */
  if (resources->sync_event_descriptor) free(resources->sync_event_descriptor);
  ...
  if (tmp_result != DOCA_SUCCESS) {
    DOCA_LOG_ERR("Failed to clean up files: %s",
                 doca_error_get_descr(tmp_result));
    DOCA_ERROR_PROPAGATE(result, tmp_result);
  }
...
doca_error_t write_file(const char *file_path, const char *string,
                        size_t string_len) {
  FILE *fp;
  doca_error_t result = DOCA_SUCCESS;
  ...
  if (fp) {
    DOCA_LOG_ERR(
        "File %s already exists. Please delete it prior to running the sample",
        file_path);
    fclose(fp);
...
doca_error_t read_file(const char *file_path, char **string,
                       size_t *string_len) {
  FILE *fp;
  long file_size;
  doca_error_t result = DOCA_SUCCESS;
...
doca_error_t rdma_cm_connect(struct rdma_resources *resources) {
  ...
  if (cfg->use_rdma_cm == true) {
    ...
    /* If cmdline option has a cm_addr param, the test object is used as a
    ...
    if ((resources->is_client == true) && (cfg->num_connections > 1)) {
      DOCA_LOG_ERR(
          "Client only support single connection, but input num_connections is "
          "[%u]",
          cfg->num_connections);
      return DOCA_ERROR_INVALID_VALUE;
    }
  ...
  DOCA_LOG_INFO("-- Connection Role: %s",
                (resources->is_client == true) ? CLIENT_NAME : SERVER_NAME);
  DOCA_LOG_INFO("-- Addr_type : %d", cfg->cm_addr_type);
  DOCA_LOG_INFO("-- Addr: %s",
                (cfg->cm_addr[0] == '\0') ? "NULL" : cfg->cm_addr);
  ...
  if (resources->is_client == false) {
    ...
    result = doca_rdma_addr_create(cfg->cm_addr_type, cfg->cm_addr,
                                   cfg->cm_port, &resources->cm_addr);
    if (DOCA_IS_ERROR(result)) {
    ...
    if (resources->cm_addr == NULL) {
      DOCA_LOG_ERR(
          "RDMA_CM client must be given a valid server address (ipv4, ipv6 or "
          "gid)");
      return DOCA_ERROR_INVALID_VALUE;
    }
  ...
  if (result != DOCA_SUCCESS)
    DOCA_LOG_ERR("[%s] failed to start connection: %s", resources->self_name,
                 doca_error_get_descr(result));
  else
...
doca_error_t rdma_cm_disconnect(struct rdma_resources *resources) {
  doca_error_t result = DOCA_SUCCESS;
  uint32_t i = 0;
  DOCA_LOG_INFO("rdma_cm_disconnect 函数被调用, 资源数量: %u",
  ...
  for (i = 0; i < resources->num_connection_established; i++) {
    ...
    if (result != DOCA_SUCCESS) {
      DOCA_LOG_ERR("[%s] cannot disconnect rdma-cm connection: %s",
                   resources->self_name, doca_error_get_descr(result));
      break;
    } else
      DOCA_LOG_INFO("[%s] successfully disconnect rdma-cm connection",
                    resources->self_name);
  }
  return result;
}
...
doca_error_t send_msg(struct doca_rdma *rdma,
                      ...
                      struct doca_buf_inventory *buf_inv, void *msg,
  ...
  result =
      doca_buf_inventory_buf_get_by_data(buf_inv, mmap, msg, msg_len, &src_buf);
  if (DOCA_IS_ERROR(result)) {
    DOCA_LOG_ERR("Failed to get a doca_buf, with error: %s",
                 doca_error_get_descr(result));
    return result;
  }

  result = doca_rdma_task_send_allocate_init(rdma, rdma_connection, src_buf,
                                             task_user_data, &rdma_send_task);
  if (DOCA_IS_ERROR(result)) {
    DOCA_LOG_ERR("Failed to allocate send task, with error: %s",
                 doca_error_get_descr(result));
    return result;
  }
  ...
  if (DOCA_IS_ERROR(result)) {
    DOCA_LOG_ERR("Failed to submit a send task, with error: %s",
                 doca_error_get_descr(result));
    return result;
  }
...
doca_error_t recv_msg(struct doca_rdma *rdma, struct doca_mmap *mmap,
                      struct doca_buf_inventory *buf_inv, void *msg,
                      uint32_t msg_len, void *user_data) {
  doca_error_t result;
  ...
  result =
      doca_buf_inventory_buf_get_by_addr(buf_inv, mmap, msg, msg_len, &dst_buf);
  if (DOCA_IS_ERROR(result)) {
    DOCA_LOG_ERR("Failed to get a doca_buf, with error: %s",
                 doca_error_get_descr(result));
    return result;
  ...
  result = doca_rdma_task_receive_allocate_init(rdma, dst_buf, task_user_data,
                                                &rdma_recv_task);
  if (DOCA_IS_ERROR(result)) {
    DOCA_LOG_ERR("Failed to allocate receive task, with error: %s",
                 doca_error_get_descr(result));
    return result;
  }
  ...
  if (DOCA_IS_ERROR(result)) {
    DOCA_LOG_ERR("Failed to submit a receive task, with error: %s",
                 doca_error_get_descr(result));
    return result;
  }
...
doca_error_t rdma_requester_recv_data_from_rdma_responder(
    struct rdma_resources *resources) {
  doca_error_t result = DOCA_SUCCESS;
  ...
  if (recv_descriptor == NULL) {
    DOCA_LOG_ERR("Failed to allocate buffer for receive descriptor: %s",
                 doca_error_get_descr(result));
    return DOCA_ERROR_NO_MEMORY;
  }
  ...
  result = create_local_mmap(&recv_descriptor_mmap,
                             DOCA_ACCESS_FLAG_LOCAL_READ_WRITE, recv_descriptor,
                             recv_descriptor_size, resources->doca_device);
  if (result != DOCA_SUCCESS) {
    DOCA_LOG_ERR("Failed to create receive descriptor's mmap: %s",
                 doca_error_get_descr(result));
    goto free_recv_descriptor;
  }

  result =
      recv_msg(resources->rdma, recv_descriptor_mmap, resources->buf_inventory,
               recv_descriptor, recv_descriptor_size, resources);
  if (result != DOCA_SUCCESS) {
    DOCA_LOG_ERR("Failed to recvd responder's data to requester: %s",
                 doca_error_get_descr(result));
    goto destroy_recv_descriptor_mmap;
  }

  //  send_flag(resources->ctrl->sockfd, CTRL_RECV_READY);
  send_flag(resources->ctrl, CTRL_RECV_READY);
...
doca_error_t rdma_responder_send_data_to_rdma_requester(
    struct rdma_resources *resources) {
  doca_error_t result = DOCA_SUCCESS;
  ...
  if (resources->recv_sync_event_desc == true) {
    ...
    result = doca_sync_event_export_to_remote_net(
        resources->sync_event,
        (const uint8_t **)&(resources->sync_event_descriptor),
        &(resources->sync_event_descriptor_size));
    if (result != DOCA_SUCCESS) {
      DOCA_LOG_ERR("Failed to export DOCA sync event for RDMA: %s",
                   doca_error_get_descr(result));
      return result;
    }
  ...
  result = create_local_mmap(&send_descriptor_mmap,
                             DOCA_ACCESS_FLAG_LOCAL_READ_WRITE, send_descriptor,
                             send_descriptor_size, resources->doca_device);
  if (result != DOCA_SUCCESS) {
    DOCA_LOG_ERR("Failed to create send mmap for local descriptor: %s",
                 doca_error_get_descr(result));
    goto destroy_buf_inv;
  }
  if (resources->recv_sync_event_desc == true)
   ...
   */
  DOCA_LOG_INFO(
      "Wait till the requester has finished the submission of the receive task "
      "for negotiation and press enter");
  wait_for_enter();

  result = send_msg(resources->rdma, resources->connections[0],
                    send_descriptor_mmap, resources->buf_inventory,
                    send_descriptor, send_descriptor_size, resources);
  if (result != DOCA_SUCCESS) {
    DOCA_LOG_ERR("Failed to send responder's data to requester: %s",
                 doca_error_get_descr(result));
...
void receive_task_completion_cb(struct doca_rdma_task_receive *task,
  ...
  // DOCA_LOG_INFO("函数 receive_task_completion_cb 被调用！！！！！！！");
  (void)task_user_data;
  ...
  if (result != DOCA_SUCCESS) {
    DOCA_LOG_ERR("Failed to get data length from doca_buf with error: %s",
  ...
  if (cm_error_occur == false) {
    ...
    if (result != DOCA_SUCCESS) cm_error_occur = true;
  }
  if (cm_error_occur == true) {
    (void)doca_ctx_stop(resource->rdma_ctx);
    DOCA_LOG_INFO("receive_task_completion_cb 调用失败");
  }
}
...
void receive_task_error_cb(struct doca_rdma_task_receive *rdma_recv_task,
  ...
  DOCA_LOG_ERR("RDMA negotiation receive task failed: %s",
               doca_error_get_descr(doca_task_get_status(task)));
  doca_task_free(task);
  doca_buf_dec_refcount(doca_rdma_task_receive_get_dst_buf(rdma_recv_task),
                        NULL);
  (void)doca_ctx_stop(resource->rdma_ctx);
  DOCA_LOG_INFO("receive_task_error_cb 调用失败");
}

void send_task_completion_cb(struct doca_rdma_task_send *task,
                             union doca_data task_user_data,
                             union doca_data ctx_user_data) {
  // DOCA_LOG_INFO("函数 send_task_completion_cb 被调用！！！！！！！");
  (void)task_user_data;
  struct rdma_resources *resource = (struct rdma_resources *)ctx_user_data.ptr;
  doca_error_t result;
  ...
  doca_buf_dec_refcount(
      (struct doca_buf *)(doca_rdma_task_send_get_src_buf(task)), NULL);

  result = resource->task_fn(resource);
  if (result != DOCA_SUCCESS) (void)doca_ctx_stop(resource->rdma_ctx);
}
...
void send_task_error_cb(struct doca_rdma_task_send *rdma_send_task,
  ...
  /* Get the result of the task */
  DOCA_LOG_ERR("RDMA negotiation send task failed: %s",
               doca_error_get_descr(doca_task_get_status(task)));
  doca_task_free(task);
  doca_buf_dec_refcount(
      (struct doca_buf *)(doca_rdma_task_send_get_src_buf(rdma_send_task)),
      NULL);
  (void)doca_ctx_stop(resource->rdma_ctx);
}
...
void rdma_cm_connect_request_cb(struct doca_rdma_connection *connection,
                                union doca_data ctx_user_data) {
  DOCA_LOG_INFO("进入 rdma_cm_connect_request_cb");
  struct rdma_resources *resource = (struct rdma_resources *)ctx_user_data.ptr;
  doca_error_t result;
  ...
  if (result != DOCA_SUCCESS) {
    DOCA_LOG_ERR("Failed to accept rdma cm connection: %s",
                 doca_error_get_descr(result));
    (void)doca_ctx_stop(resource->rdma_ctx);
    return;
  }
  ...
  if (result != DOCA_SUCCESS) {
    DOCA_LOG_ERR("Failed to set server connection user data: %s",
                 doca_error_get_descr(result));
    (void)doca_ctx_stop(resource->rdma_ctx);
  }
  DOCA_LOG_INFO("RDMA connection request accepted 成功");
}
...
void rdma_cm_connect_established_cb(struct doca_rdma_connection *connection,
  ...
  resource->connection_established[resource->num_connection_established++] =
      true;

  if (resource->require_remote_mmap == false) {
    if (resource->num_connection_established >=
        resource->cfg->num_connections) {
      if (resource->task_fn(resource) != DOCA_SUCCESS) {
        (void)doca_ctx_stop(resource->rdma_ctx);
        DOCA_LOG_ERR("失败 Failed to call task function");
      }
    }
    return;
  }
...
void rdma_cm_connect_failure_cb(struct doca_rdma_connection *connection,
  ...
  DOCA_ERROR_PROPAGATE(resource->first_encountered_error,
                       DOCA_ERROR_CONNECTION_ABORTED);
  (void)doca_ctx_stop(resource->rdma_ctx);
  DOCA_LOG_INFO("rdma_cm_connect_failure_cb 调用");
}
...
void rdma_cm_disconnect_cb(struct doca_rdma_connection *connection,
  ...
  if (result != DOCA_SUCCESS) {
    DOCA_LOG_ERR("Failed to disconnect rdma cm connection: %s",
                 doca_error_get_descr(result));
    (void)doca_ctx_stop(resource->rdma_ctx);
    return;
  }
...
doca_error_t create_local_mmap(struct doca_mmap **mmap,
  ...
  if (result != DOCA_SUCCESS) {
    DOCA_LOG_ERR("Failed to create mmap for source buffer, error: %s",
                 doca_error_get_descr(result));
    return result;
  }
  ...
  if (result != DOCA_SUCCESS) {
    DOCA_LOG_ERR("Failed to set mmap source buffer permissions, error: %s",
                 doca_error_get_descr(result));
    goto destroy_mmap;
  }
  ...
  if (result != DOCA_SUCCESS) {
    DOCA_LOG_ERR("Failed to set memory range, error: %s",
                 doca_error_get_descr(result));
    goto destroy_mmap;
  }
  ...
  if (result != DOCA_SUCCESS) {
    DOCA_LOG_ERR("Failed to add device to mmap, error: %s",
                 doca_error_get_descr(result));
    goto destroy_mmap;
  }
  ...
  if (result != DOCA_SUCCESS) {
    DOCA_LOG_ERR("Failed to start mmap, error: %s",
                 doca_error_get_descr(result));
    goto destroy_mmap;
  }
...
doca_error_t config_rdma_cm_callback_and_negotiation_task(
    struct rdma_resources *resources,
    bool need_send_task,  // false
    bool need_recv_task)  // true
{
  doca_error_t result = DOCA_SUCCESS;

  if (resources == NULL) {
    result = DOCA_ERROR_INVALID_VALUE;
    DOCA_LOG_ERR("Invalid resource pointer found, error: %s",
                 doca_error_get_descr(result));
    return result;
  }
   ...
   * they are used for transferring the mmap desc between client and server for
   * non-sync-event task also used for transferring the sync-event desc between
   * client and server for syn-event task
   */
  if (need_recv_task == true) {
    result = doca_rdma_task_receive_set_conf(
        resources->rdma, receive_task_completion_cb, receive_task_error_cb,
        NUM_NEGOTIATION_RDMA_TASKS);
    if (result != DOCA_SUCCESS) {
      DOCA_LOG_ERR("Failed to set task recv configuration, error: %s",
                   doca_error_get_descr(result));
      return result;
    }
  }
  if (need_send_task == true) {
    result = doca_rdma_task_send_set_conf(
        resources->rdma, send_task_completion_cb, send_task_error_cb,
        NUM_NEGOTIATION_RDMA_TASKS);
    if (result != DOCA_SUCCESS) {
      DOCA_LOG_ERR("Failed to set task send configuration, error: %s",
                   doca_error_get_descr(result));
      return result;
    }
  }
  ...
  result = doca_rdma_set_connection_state_callbacks(
      resources->rdma, rdma_cm_connect_request_cb,
      rdma_cm_connect_established_cb, rdma_cm_connect_failure_cb,
      rdma_cm_disconnect_cb);
  if (result != DOCA_SUCCESS) {
    DOCA_LOG_ERR("Failed to set rdma cm callback configuration, error: %s",
                 doca_error_get_descr(result));
    return result;
  }

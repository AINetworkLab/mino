...
// #define MEM_RANGE_LEN (1024*1024*1024)		     /* DOCA mmap memory range length */
#define MEM_RANGE_LEN (4096)		     /* DOCA mmap memory range length */
...
// ws  任务数量设置 默认为 1 
#define NUM_RDMA_TASKS (50)	   /* Number of RDMA tasks*/
...
struct rdma_resources {
    ...
    void **src_buf_datas;
    struct doca_rdma_task_write **rdma_write_tasks;

	struct my_shm_region *reg;
	char mmap_desc_path[MAX_DESC_PATH]; /* Path to save the local mmap descriptor */
	struct control_channel *ctrl;
